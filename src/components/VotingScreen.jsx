...
export default function VotingScreen({ players, selectedVote, onVoteSelect, onConfirm, onBack }) {
      ...
      <div className="flex flex-col gap-5 w-full max-w-sm pt-8">
        ...
        <div className="text-center mb-2">
          <div className="w-16 h-16 bg-gray-50 rounded-full flex items-center justify-center mx-auto mb-3">
            <Users size={32} className="text-black" strokeWidth={1.5} />
          </div>
          <h2 className="text-2xl font-bold text-black">رأی‌گیری</h2>
          <p className="text-sm text-gray-400 mt-1">
            به کسی که فکر می‌کنید جاسوس است رأی دهید
          </p>
        </div>
          ...
          {players.map((player, i) => (
            ...
            >
              <span className={`w-10 h-10 rounded-full flex items-center justify-center text-sm font-bold flex-shrink-0
                ${selectedVote === i ? 'bg-white/20 text-white' : 'bg-white text-gray-400'}`}
              >
                {toPersian(i + 1)}
              </span>
